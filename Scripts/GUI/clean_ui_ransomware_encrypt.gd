...
@export var ransomware_window :Control
...
func set_step(step: int) -> void:
	...
	match step:
		1:
			...
			rich_text_label.text = "Step 1: Importing the tools\n\nFirst, the program imports two important tools:\n\n\"os\": This lets the program interact with the computer’s files and folders. Think of it as a helper that can look inside directories, check whether something is a file, and manage paths."
			animation.play("decrypt_ransomware_step1")
			
		2:
			rich_text_label.text = "Step 1: Importing the tools\n\nFirst, the program imports two important tools:\n\n\"Fernet from cryptography\"\nThis is a secure encryption system. You can imagine it like a lock-and-key system: it can lock data (encrypt it) so nobody can read it, and later unlock it (decrypt it) if you have the secret key."
			animation.play("decrypt_ransomware_step1.2")
			
		3:
			code_edit.text = ransomware_code.step_1 + "\n\n" + ransomware_code.step_2
			rich_text_label.text = "Step 2: Collecting Files to Encypt\n\n- It starts with an empty list called allfiles."
		...
		8:
			...
			rich_text_label.text = "Step 3: Creating the Key\n\nNow comes the most important part — making the key:\n\nThe program calls \"Fernet.generate_key()\", which creates a brand-new secret key."
		...
		14:
			...
			rich_text_label.text = "Step 5: Final Confirmation\n\nAfter finishing the encryption process, the program prints:"
		...
		16:	
			ransomware_window.visible = true
